...
|---|---|---|
| Message transcripts | `data/transcripts/<waNumber>.jsonl` (or Supabase when `DATABASE_URL` is set) | Agent desk chat history |
...
**Render free disk is ephemeral** — redeploys wipe `data/` unless you attach a persistent disk. **Keep chat history forever (free):** create a [Supabase](https://supabase.com) project and follow [docs/SUPABASE_SETUP.md](docs/SUPABASE_SETUP.md) — set `DATABASE_URL` on Render.
